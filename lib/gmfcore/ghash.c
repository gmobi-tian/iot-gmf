...
g_hash_table_lookup_node (GHashTable	*hash_table,
						  gconstpointer	 key);
static int
g_hash_table_lookup_index (GHashTable	*hash_table,
						   gconstpointer	 key);
		  ...
		  GCompareFunc key_compare_func)
{
  ...
  hash_table->nodes = g_array_new();
  
  return hash_table;
}

static void g_hash_table_deep_destroy_items(gpointer  key,
											  gpointer  value,
											  gpointer  user_data)
{
	g_free(key);
...
{  
	...
	if (i >= 0)
		g_array_remove_index(hash_table->nodes, GHashNode, i);
}
		      ...
		      gpointer    user_data)
{
  ...
  for (i = 0; i < size; i++)
  {
	  GHashNode *node = &g_array_index(hash_table->nodes, GHashNode, i);
...
gint g_hash_table_size (GHashTable *hash_table)
{
  ...
  return g_array_length(hash_table->nodes, GHashNode);
}
			  ...
			  gconstpointer	 key)
{
  ...
  {
	  m = (l + r) / 2;
	  node = &g_array_index(hash_table->nodes, GHashNode, m);
	  if (hash == node->hash) 
		  return m;
						  ...
						  gconstpointer	 key)
{
	...
	return i == -1 ? NULL : &g_array_index(hash_table->nodes, GHashNode, i);
}
void g_hash_table_insert (GHashTable *hash_table, gpointer key, gpointer value)
{
	...
	if (node == NULL)
		g_array_append_val(hash_table->nodes, GHashNode, nodenew);
	else
		g_array_insert_val(hash_table->nodes, hash < node->hash ? m : m + 1, GHashNode, nodenew);
}
