...
static VMINT coroutine_initialized = FALSE;
...
static void _on_coroutine_process(VMINT tid)
{
...
void g_coroutine_initialize(void)
{
	...
	{
		gpointer coroutine_timer_id = g_create_timer(20, _on_coroutine_process);
		g_log_debug("g_coroutine_initialize : %d", *((int *) coroutine_timer_id));
