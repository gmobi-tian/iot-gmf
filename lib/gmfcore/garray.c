...
GArray*
g_rarray_set_length (GArray *array,
		   gint    length,
		   gint    size)
{
							   ...
							   gint		 index)
{
	if (array->data && index < array->len / size -1)
		g_memmove(array->data + index * size, array->data + (index + 1) * size, array->len - (1 + index) * size);
	array->len -= size;
	return array;
}
						 ...
						 gint      size)
{
	...
	if (array->data && index < array->len / size -1)
		g_memmove(array->data + (index + 1) * size, array->data + index * size, array->len - (1 + index) * size);
...
g_ptr_array_new ()
{
	return g_ptr_array_new_sorted(NULL);
}
...
void
g_ptr_array_set_capacity  (GPtrArray   *farray,
					   gint	     length)
{
...
gint g_ptr_array_index_of(GPtrArray	*ar, void* item)
{
	...
	{
		if (item == g_ptr_array_index(ar, i)) return i;
	}
	return -1;
}
gboolean
